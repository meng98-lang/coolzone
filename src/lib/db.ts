...
// settings 表为扁平结构（单行配置）：
//   whatsapp_phone, facebook_pixel_id, google_analytics_id, tiktok_pixel_id, admin_password
...
export async function getSettings(): Promise<SiteSettings> {
  ...
  const row = data[0];
  return {
    whatsappPhone: row.whatsapp_phone || DEFAULT_SETTINGS.whatsappPhone,
...
// 更新网站配置（扁平结构，upsert 到单行）
export async function updateSettings(updates: Partial<SiteSettings>): Promise<SiteSettings> {
  const patch: { [key: string]: string } = {};
  if (updates.whatsappPhone !== undefined) patch.whatsapp_phone = updates.whatsappPhone;
  if (updates.facebookPixelId !== undefined) patch.facebook_pixel_id = updates.facebookPixelId;
  if (updates.googleAnalyticsId !== undefined) patch.google_analytics_id = updates.googleAnalyticsId;
  if (updates.tiktokPixelId !== undefined) patch.tiktok_pixel_id = updates.tiktokPixelId;
  if (updates.adminPassword !== undefined) patch.admin_password = updates.adminPassword;
